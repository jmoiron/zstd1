...
/*!
 * HEAPMODE :
 * Select how default decompression function ZSTD1_decompress() allocates its context,
 * on stack (0), or into heap (1, default; requires malloc()).
 * Note that functions with explicit context such as ZSTD1_decompressDCtx() are unaffected.
 */
 ...
 *  It's possible to set a different limit using ZSTD1_DCtx_setMaxWindowSize().
 */
#ifndef ZSTD1_MAXWINDOWSIZE_DEFAULT
#  define ZSTD1_MAXWINDOWSIZE_DEFAULT (((U32)1 << ZSTD1_WINDOWLOG_DEFAULTMAX) + 1)
#endif
...
typedef enum { ZSTDds_getFrameHeaderSize, ZSTDds_decodeFrameHeader,
               ...
               ZSTDds_decodeSkippableHeader, ZSTDds_skipFrame } ZSTD1_dStage;
...
typedef struct {
    ZSTD1_seqSymbol LLTable[SEQSYMBOL_TABLE_SIZE(LLFSELog)];
    ZSTD1_seqSymbol OFTable[SEQSYMBOL_TABLE_SIZE(OffFSELog)];
    ZSTD1_seqSymbol MLTable[SEQSYMBOL_TABLE_SIZE(MLFSELog)];
...
} ZSTD1_entropyDTables_t;

struct ZSTD1_DCtx_s
{
    const ZSTD1_seqSymbol* LLTptr;
    const ZSTD1_seqSymbol* MLTptr;
    const ZSTD1_seqSymbol* OFTptr;
    const HUF1_DTable* HUFptr;
    ZSTD1_entropyDTables_t entropy;
    ...
    U64 decodedSize;
    blockType_e bType;            /* used in ZSTD1_decompressContinue(), store blockType between block header decoding and block decompression stages */
    ZSTD1_dStage stage;
    U32 litEntropy;
    U32 fseEntropy;
...
static size_t ZSTD1_startingInputLength(ZSTD1_format_e format)
{
    size_t const startingInputLength = (format==ZSTD1_f_zstd1_magicless) ?
                    ZSTD1_frameHeaderSize_prefix - ZSTD1_frameIdSize :
                    ZSTD1_frameHeaderSize_prefix;
    ZSTD1_STATIC_ASSERT(ZSTD1_FRAMEHEADERSIZE_PREFIX >= ZSTD1_FRAMEIDSIZE);
    /* only supports formats ZSTD1_f_zstd1 and ZSTD1_f_zstd1_magicless */
    assert( (format == ZSTD1_f_zstd1) || (format == ZSTD1_f_zstd1_magicless) );
    return startingInputLength;
}

static void ZSTD1_initDCtx_internal(ZSTD1_DCtx* dctx)
{
    dctx->format = ZSTD1_f_zstd1;  /* ZSTD1_decompressBegin() invokes ZSTD1_startingInputLength() with argument dctx->format */
    dctx->staticSize  = 0;
    ...
    dctx->bmi2 = ZSTD1_cpuid_bmi2(ZSTD1_cpuid());
}

ZSTD1_DCtx* ZSTD1_initStaticDCtx(void *workspace, size_t workspaceSize)
{
    ZSTD1_DCtx* const dctx = (ZSTD1_DCtx*) workspace;
...
{
    if (size < ZSTD1_frameIdSize) return 0;
...
/** ZSTD1_frameHeaderSize_internal() :
 *  srcSize must be large enough to reach header size fields.
 *  note : only works for formats ZSTD1_f_zstd1 and ZSTD1_f_zstd1_magicless.
 * @return : size of the Frame Header
 *           or an error code, which can be tested with ZSTD1_isError() */
static size_t ZSTD1_frameHeaderSize_internal(const void* src, size_t srcSize, ZSTD1_format_e format)
{
    size_t const minInputSize = ZSTD1_startingInputLength(format);
    ...
    {   BYTE const fhd = ((const BYTE*)src)[minInputSize-1];
        ...
        return minInputSize + !singleSegment
             + ZSTD1_did_fieldSize[dictID] + ZSTD1_fcs_fieldSize[fcsId]
             + (singleSegment && !fcsId);
    }
}
...
{
    return ZSTD1_frameHeaderSize_internal(src, srcSize, ZSTD1_f_zstd1);
}


/** ZSTD1_getFrameHeader_internal() :
 *  decode Frame Header, or require larger `srcSize`.
 *  note : only works for formats ZSTD1_f_zstd1 and ZSTD1_f_zstd1_magicless
 * @return : 0, `zfhPtr` is correctly filled,
 *          >0, `srcSize` is too small, value is wanted `srcSize` amount,
 *           or an error code, which can be tested using ZSTD1_isError() */
static size_t ZSTD1_getFrameHeader_internal(ZSTD1_frameHeader* zfhPtr, const void* src, size_t srcSize, ZSTD1_format_e format)
{
    ...
    if ( (format != ZSTD1_f_zstd1_magicless)
      && (MEM_readLE32(src) != ZSTD1_MAGICNUMBER) ) {
        if ((MEM_readLE32(src) & 0xFFFFFFF0U) == ZSTD1_MAGIC_SKIPPABLE_START) {
            ...
            zfhPtr->frameContentSize = MEM_readLE32((const char *)src + ZSTD1_frameIdSize);
            zfhPtr->frameType = ZSTD1_skippableFrame;
            return 0;
        }
    ...
    {   size_t const fhsize = ZSTD1_frameHeaderSize_internal(src, srcSize, format);
    ...
    {   BYTE const fhdByte = ip[minInputSize-1];
        ...
        U64 windowSize = 0;
        U32 dictID = 0;
        U64 frameContentSize = ZSTD1_CONTENTSIZE_UNKNOWN;
        if ((fhdByte & 0x08) != 0)
        ...
        zfhPtr->frameType = ZSTD1_frame;
        zfhPtr->frameContentSize = frameContentSize;
        zfhPtr->windowSize = windowSize;
        zfhPtr->blockSizeMax = (unsigned) MIN(windowSize, ZSTD1_BLOCKSIZE_MAX);
...
{
    return ZSTD1_getFrameHeader_internal(zfhPtr, src, srcSize, ZSTD1_f_zstd1);
}
...
#endif
    {   ZSTD1_frameHeader zfh;
        if (ZSTD1_getFrameHeader(&zfh, src, srcSize) != 0)
            return ZSTD1_CONTENTSIZE_ERROR;
        if (zfh.frameType == ZSTD1_skippableFrame) {
            return 0;
        } else {
...
unsigned long long ZSTD1_findDecompressedSize(const void* src, size_t srcSize)
{
    ...
    while (srcSize >= ZSTD1_frameHeaderSize_prefix) {
        ...
        if ((magicNumber & 0xFFFFFFF0U) == ZSTD1_MAGIC_SKIPPABLE_START) {
            ...
            skippableSize = MEM_readLE32((const BYTE *)src + ZSTD1_frameIdSize)
    ...
    if (srcSize) return ZSTD1_CONTENTSIZE_ERROR;
...
{
    unsigned long long const ret = ZSTD1_getFrameContentSize(src, srcSize);
    ZSTD1_STATIC_ASSERT(ZSTD1_CONTENTSIZE_ERROR < ZSTD1_CONTENTSIZE_UNKNOWN);
...
{
    size_t const result = ZSTD1_getFrameHeader_internal(&(dctx->fParams), src, headerSize, dctx->format);
        ...
        case set_compressed:
            ...
            {   size_t lhSize, litSize, litCSize;
                ...
                if (HUF1_isError((litEncType==set_repeat) ?
                                    ( singleStream ?
                                        HUF1_decompress1X_usingDTable_bmi2(dctx->litBuffer, litSize, istart+lhSize, litCSize, dctx->HUFptr, dctx->bmi2) :
                                        HUF1_decompress4X_usingDTable_bmi2(dctx->litBuffer, litSize, istart+lhSize, litCSize, dctx->HUFptr, dctx->bmi2) ) :
                                    ( singleStream ?
                                        HUF1_decompress1X2_DCtx_wksp_bmi2(dctx->entropy.hufTable, dctx->litBuffer, litSize, istart+lhSize, litCSize,
                                                                         dctx->entropy.workspace, sizeof(dctx->entropy.workspace), dctx->bmi2) :
                                        HUF1_decompress4X_hufOnly_wksp_bmi2(dctx->entropy.hufTable, dctx->litBuffer, litSize, istart+lhSize, litCSize,
                                                                           dctx->entropy.workspace, sizeof(dctx->entropy.workspace), dctx->bmi2))))
 ...
 * - generate tables normally, using ZSTD1_buildFSETable()
 * - printout the content of tables
 * - pretify output, report below, test with fuzzer to ensure it's correct */

/* Default FSE distribution table for Literal Lengths */
static const ZSTD1_seqSymbol LL_defaultDTable[(1<<LL_DEFAULTNORMLOG)+1] = {
...
static const ZSTD1_seqSymbol OF_defaultDTable[(1<<OF_DEFAULTNORMLOG)+1] = {
...
static const ZSTD1_seqSymbol ML_defaultDTable[(1<<ML_DEFAULTNORMLOG)+1] = {
...
static void ZSTD1_buildSeqTable_rle(ZSTD1_seqSymbol* dt, U32 baseValue, U32 nbAddBits)
{
    void* ptr = dt;
    ZSTD1_seqSymbol_header* const DTableH = (ZSTD1_seqSymbol_header*)ptr;
    ZSTD1_seqSymbol* const cell = dt + 1;
...
static void
ZSTD1_buildFSETable(ZSTD1_seqSymbol* dt,
    const short* normalizedCounter, unsigned maxSymbolValue,
    const U32* baseValue, const U32* nbAdditionalBits,
    unsigned tableLog)
{
    ZSTD1_seqSymbol* const tableDecode = dt+1;
    ...
    {   ZSTD1_seqSymbol_header DTableH;
...
static size_t ZSTD1_buildSeqTable(ZSTD1_seqSymbol* DTableSpace, const ZSTD1_seqSymbol** DTablePtr,
                                 symbolEncodingType_e type, U32 max, U32 maxLog,
                                 const void* src, size_t srcSize,
                                 const U32* baseValue, const U32* nbAdditionalBits,
                                 const ZSTD1_seqSymbol* defaultTable, U32 flagRepeatTable)
{
    switch(type)
    {
    case set_rle :
        ...
        {   U32 const symbol = *(const BYTE*)src;
            ...
            ZSTD1_buildSeqTable_rle(DTableSpace, baseline, nbBits);
        }
    ...
    case set_compressed :
        {   U32 tableLog;
            ...
            ZSTD1_buildFSETable(DTableSpace, norm, max, baseValue, nbAdditionalBits, tableLog);
            *DTablePtr = DTableSpace;
            return headerSize;
        }
    default :   /* impossible */
...
typedef struct {
    size_t state;
    const ZSTD1_seqSymbol* table;
} ZSTD1_fseState;

typedef struct {
    BIT_DStream_t DStream;
    ZSTD1_fseState stateLL;
    ZSTD1_fseState stateOffb;
    ZSTD1_fseState stateML;
                             ...
                             const BYTE* const prefixStart, const BYTE* const dictStart, const BYTE* const dictEnd)
{
    ...
    if (oMatchEnd > oend) return ERROR(dstSize_tooSmall); /* last match must start at a minimum distance of WILDCOPY_OVERLENGTH from oend */
    if (iLitEnd > litLimit) return ERROR(corruption_detected);   /* over-read beyond lit buffer */
    if (oLitEnd > oend_w) return ZSTD1_execSequenceLast7(op, oend, sequence, litPtr, litLimit, prefixStart, dictStart, dictEnd);

    /* copy Literals */
    ZSTD1_copy8(op, *litPtr);  /* note : op <= oLitEnd <= oend_w == oend - 8 */
    if (sequence.litLength > 8)
        ZSTD1_wildcopy(op+8, (*litPtr)+8, sequence.litLength - 8);   /* note : since oLitEnd <= oend-WILDCOPY_OVERLENGTH, no risk of overwrite beyond oend */
    op = oLitEnd;
    *litPtr = iLitEnd;   /* update for next sequence */
...
static void
ZSTD1_initFseState(ZSTD1_fseState* DStatePtr, BIT_DStream_t* bitD, const ZSTD1_seqSymbol* dt)
{
    const void* ptr = dt;
    const ZSTD1_seqSymbol_header* const DTableH = (const ZSTD1_seqSymbol_header*)ptr;
    DStatePtr->state = BIT_readBits(bitD, DTableH->tableLog);
    DEBUGLOG(6, "ZSTD1_initFseState : val=%u using %u bits",
...
ZSTD1_updateFseState(ZSTD1_fseState* DStatePtr, BIT_DStream_t* bitD)
{
    ZSTD1_seqSymbol const DInfo = DStatePtr->table[DStatePtr->state];
    U32 const nbBits = DInfo.nbBits;
    size_t const lowBits = BIT_readBits(bitD, nbBits);
    DStatePtr->state = DInfo.nextState + lowBits;
}

/* We need to add at most (ZSTD1_WINDOWLOG_MAX_32 - 1) bits to read the maximum
 * offset bits. But we can only read at most (STREAM_ACCUMULATOR_MIN_32 - 1)
 * bits before reloading. This value is the maximum number of bytes we read
 * after reloading when we are decoding long offets.
 */
#define LONG_OFFSETS_MAX_EXTRA_BITS_32                       \
    (ZSTD1_WINDOWLOG_MAX_32 > STREAM_ACCUMULATOR_MIN_32       \
        ? ZSTD1_WINDOWLOG_MAX_32 - STREAM_ACCUMULATOR_MIN_32  \
        : 0)

typedef enum { ZSTD1_lo_isRegularOffset, ZSTD1_lo_isLongOffset=1 } ZSTD1_longOffset_e;

FORCE_INLINE_TEMPLATE seq_t
ZSTD1_decodeSequence(seqState_t* seqState, const ZSTD1_longOffset_e longOffsets)
{
    seq_t seq;
    ...
    {   size_t offset;
        if (!ofBits)
            ...
            ZSTD1_STATIC_ASSERT(ZSTD1_lo_isLongOffset == 1);
            ZSTD1_STATIC_ASSERT(LONG_OFFSETS_MAX_EXTRA_BITS_32 == 5);
            ...
            if (MEM_32bits() && longOffsets && (ofBits >= STREAM_ACCUMULATOR_MIN_32)) {
                ...
                offset = ofBase + BIT_readBitsFast(&seqState->DStream, ofBits/*>0*/);   /* <=  (ZSTD1_WINDOWLOG_MAX-1) bits */
    ...
    ZSTD1_STATIC_ASSERT(16+LLFSELog+MLFSELog+OffFSELog < STREAM_ACCUMULATOR_MIN_64);
    ...
    /* ANS state update */
    ZSTD1_updateFseState(&seqState->stateLL, &seqState->DStream);    /* <=  9 bits */
    ZSTD1_updateFseState(&seqState->stateML, &seqState->DStream);    /* <=  9 bits */
    if (MEM_32bits()) BIT_reloadDStream(&seqState->DStream);    /* <= 18 bits */
    ZSTD1_updateFseState(&seqState->stateOffb, &seqState->DStream);  /* <=  8 bits */

    return seq;
}

FORCE_INLINE_TEMPLATE size_t
ZSTD1_decompressSequences_body( ZSTD1_DCtx* dctx,
                               void* dst, size_t maxDstSize,
                         const void* seqStart, size_t seqSize, int nbSeq,
                         const ZSTD1_longOffset_e isLongOffset)
{
    ...
    if (nbSeq) {
        ...
        ZSTD1_initFseState(&seqState.stateLL, &seqState.DStream, dctx->LLTptr);
        ZSTD1_initFseState(&seqState.stateOffb, &seqState.DStream, dctx->OFTptr);
        ZSTD1_initFseState(&seqState.stateML, &seqState.DStream, dctx->MLTptr);

        for ( ; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) && nbSeq ; ) {
            nbSeq--;
            {   seq_t const sequence = ZSTD1_decodeSequence(&seqState, isLongOffset);
        ...
        DEBUGLOG(5, "ZSTD1_decompressSequences: after decode loop, remaining nbSeq : %i", nbSeq);
...
static size_t
ZSTD1_decompressSequences_default(ZSTD1_DCtx* dctx,
                                 void* dst, size_t maxDstSize,
                           const void* seqStart, size_t seqSize, int nbSeq,
                           const ZSTD1_longOffset_e isLongOffset)
{
    return ZSTD1_decompressSequences_body(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
}



FORCE_INLINE_TEMPLATE seq_t
ZSTD1_decodeSequenceLong(seqState_t* seqState, ZSTD1_longOffset_e const longOffsets)
{
    seq_t seq;
    U32 const llBits = seqState->stateLL.table[seqState->stateLL.state].nbAdditionalBits;
    U32 const mlBits = seqState->stateML.table[seqState->stateML.state].nbAdditionalBits;
    ...
    {   size_t offset;
        if (!ofBits)
            ...
            if (MEM_32bits() && longOffsets) {
                ...
                offset = ofBase + BIT_readBitsFast(&seqState->DStream, ofBits);   /* <=  (ZSTD1_WINDOWLOG_MAX-1) bits */
                if (MEM_32bits()) BIT_reloadDStream(&seqState->DStream);
            }
        }
    ...
    {   size_t const pos = seqState->pos + seq.litLength;
        ...
        seq.match = matchBase + pos - seq.offset;  /* note : this operation can overflow when seq.offset is really too large, which can only happen when input is corrupted.
                                                    * No consequence though : no memory access will occur, overly large offset will be detected in ZSTD1_execSequenceLong() */
...
FORCE_INLINE_TEMPLATE size_t
ZSTD1_decompressSequencesLong_body(
                               ZSTD1_DCtx* dctx,
                               void* dst, size_t maxDstSize,
                         const void* seqStart, size_t seqSize, int nbSeq,
                         const ZSTD1_longOffset_e isLongOffset)
...
#define ADVANCED_SEQS 4
        ...
        for (seqNb=0; (BIT_reloadDStream(&seqState.DStream) <= BIT_DStream_completed) && (seqNb<seqAdvance); seqNb++) {
            sequences[seqNb] = ZSTD1_decodeSequenceLong(&seqState, isLongOffset);
        }
        if (seqNb<seqAdvance) return ERROR(corruption_detected);

        /* decode and decompress */
        for ( ; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) && (seqNb<nbSeq) ; seqNb++) {
            seq_t const sequence = ZSTD1_decodeSequenceLong(&seqState, isLongOffset);
            size_t const oneSeqSize = ZSTD1_execSequenceLong(op, oend, sequences[(seqNb-ADVANCED_SEQS) & STOSEQ_MASK], &litPtr, litEnd, prefixStart, dictStart, dictEnd);
            if (ZSTD1_isError(oneSeqSize)) return oneSeqSize;
        ...
        for ( ; seqNb<nbSeq ; seqNb++) {
            size_t const oneSeqSize = ZSTD1_execSequenceLong(op, oend, sequences[seqNb&STOSEQ_MASK], &litPtr, litEnd, prefixStart, dictStart, dictEnd);
            if (ZSTD1_isError(oneSeqSize)) return oneSeqSize;
            op += oneSeqSize;
        }
...
static size_t
ZSTD1_decompressSequencesLong_default(ZSTD1_DCtx* dctx,
                                 void* dst, size_t maxDstSize,
                           const void* seqStart, size_t seqSize, int nbSeq,
                           const ZSTD1_longOffset_e isLongOffset)
{
    return ZSTD1_decompressSequencesLong_body(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
}
...
ZSTD1_decompressSequences_bmi2(ZSTD1_DCtx* dctx,
                                 void* dst, size_t maxDstSize,
                           const void* seqStart, size_t seqSize, int nbSeq,
                           const ZSTD1_longOffset_e isLongOffset)
{
...
static TARGET_ATTRIBUTE("bmi2") size_t
ZSTD1_decompressSequencesLong_bmi2(ZSTD1_DCtx* dctx,
                                 void* dst, size_t maxDstSize,
                           const void* seqStart, size_t seqSize, int nbSeq,
                           const ZSTD1_longOffset_e isLongOffset)
{
...
typedef size_t (*ZSTD1_decompressSequences_t)(
    ZSTD1_DCtx *dctx, void *dst, size_t maxDstSize,
    const void *seqStart, size_t seqSize, int nbSeq,
    const ZSTD1_longOffset_e isLongOffset);

static size_t ZSTD1_decompressSequences(ZSTD1_DCtx* dctx, void* dst, size_t maxDstSize,
                                const void* seqStart, size_t seqSize, int nbSeq,
                                const ZSTD1_longOffset_e isLongOffset)
{
    DEBUGLOG(5, "ZSTD1_decompressSequences");
#if DYNAMIC_BMI2
    if (dctx->bmi2) {
        return ZSTD1_decompressSequences_bmi2(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
    }
#endif
  return ZSTD1_decompressSequences_default(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
}

static size_t ZSTD1_decompressSequencesLong(ZSTD1_DCtx* dctx,
                                void* dst, size_t maxDstSize,
                                const void* seqStart, size_t seqSize, int nbSeq,
                                const ZSTD1_longOffset_e isLongOffset)
{
    DEBUGLOG(5, "ZSTD1_decompressSequencesLong");
#if DYNAMIC_BMI2
    if (dctx->bmi2) {
        return ZSTD1_decompressSequencesLong_bmi2(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
    }
#endif
  return ZSTD1_decompressSequencesLong_default(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
}

/* ZSTD1_getLongOffsetsShare() :
 * condition : offTable must be valid
 * @return : "share" of long offsets (arbitrarily defined as > (1<<23))
 *           compared to maximum possible of (1<<OffFSELog) */
static unsigned
ZSTD1_getLongOffsetsShare(const ZSTD1_seqSymbol* offTable)
{
    const void* ptr = offTable;
    U32 const tableLog = ((const ZSTD1_seqSymbol_header*)ptr)[0].tableLog;
    const ZSTD1_seqSymbol* table = offTable + 1;
    U32 const max = 1 << tableLog;
    U32 u, total = 0;
    DEBUGLOG(5, "ZSTD1_getLongOffsetsShare: (tableLog=%u)", tableLog);
...
{   /* blockType == blockCompressed */
     ...
     */
    ZSTD1_longOffset_e const isLongOffset = (ZSTD1_longOffset_e)(MEM_32bits() && (!frame || dctx->fParams.windowSize > (1ULL << STREAM_ACCUMULATOR_MIN)));
    DEBUGLOG(5, "ZSTD1_decompressBlock_internal (size : %u)", (U32)srcSize);
    ...
    {   int nbSeq;
        size_t const seqHSize = ZSTD1_decodeSeqHeaders(dctx, &nbSeq, ip, srcSize);
        ...
        if ( (!frame || dctx->fParams.windowSize > (1<<24))
          && (nbSeq>0) ) {  /* could probably use a larger nbSeq limit */
            U32 const shareLongOffsets = ZSTD1_getLongOffsetsShare(dctx->OFTptr);
            U32 const minShare = MEM_64bits() ? 7 : 20; /* heuristic values, correspond to 2.73% and 7.81% */
            if (shareLongOffsets >= minShare)
                return ZSTD1_decompressSequencesLong(dctx, dst, dstCapacity, ip, srcSize, nbSeq, isLongOffset);
        }

        return ZSTD1_decompressSequences(dctx, dst, dstCapacity, ip, srcSize, nbSeq, isLongOffset);
    }
}
                      ...
                      const void* src, size_t srcSize)
{
    ...
    dSize = ZSTD1_decompressBlock_internal(dctx, dst, dstCapacity, src, srcSize, /* frame */ 0);
...
ZSTDLIB_API size_t ZSTD1_insertBlock(ZSTD1_DCtx* dctx, const void* blockStart, size_t blockSize)
{
...
static size_t ZSTD1_generateNxBytes(void* dst, size_t dstCapacity, BYTE byte, size_t length)
{
...
#if defined(ZSTD1_LEGACY_SUPPORT) && (ZSTD1_LEGACY_SUPPORT >= 1)
    if (ZSTD1_isLegacy(src, srcSize))
        return ZSTD1_findFrameCompressedSizeLegacy(src, srcSize);
#endif
    if ( (srcSize >= ZSTD1_skippableHeaderSize)
      && (MEM_readLE32(src) & 0xFFFFFFF0U) == ZSTD1_MAGIC_SKIPPABLE_START ) {
        return ZSTD1_skippableHeaderSize + MEM_readLE32((const BYTE*)src + ZSTD1_frameIdSize);
    } else {
        const BYTE* ip = (const BYTE*)src;
        ...
        {   size_t const ret = ZSTD1_getFrameHeader(&zfh, src, srcSize);
        ...
        while (1) {
            ...
            if (ZSTD1_blockHeaderSize + cBlockSize > remainingSize)
                return ERROR(srcSize_wrong);
                             ...
                             const void** srcPtr, size_t *srcSizePtr)
{
    ...
    /* check */
    if (remainingSize < ZSTD1_frameHeaderSize_min+ZSTD1_blockHeaderSize)
        return ERROR(srcSize_wrong);
    ...
    {   size_t const frameHeaderSize = ZSTD1_frameHeaderSize(ip, ZSTD1_frameHeaderSize_prefix);
        ...
        if (remainingSize < frameHeaderSize+ZSTD1_blockHeaderSize)
            return ERROR(srcSize_wrong);
    ...
    while (1) {
        ...
        {
        case bt_compressed:
            decodedSize = ZSTD1_decompressBlock_internal(dctx, op, oend-op, ip, cBlockSize, /* frame */ 1);
            break;
        case bt_raw :
    ...
    if (dctx->fParams.frameContentSize != ZSTD1_CONTENTSIZE_UNKNOWN) {
                                  ...
                                  const ZSTD1_DDict* ddict)
{
    ...
    while (srcSize >= ZSTD1_frameHeaderSize_prefix) {
        ...
        DEBUGLOG(4, "reading magic number %08X (expecting %08X)",
                    (U32)magicNumber, (U32)ZSTD1_MAGICNUMBER);
        ...
        {   const size_t res = ZSTD1_decompressFrame(dctx, dst, dstCapacity,
            ...
            /* no need to bound check, ZSTD1_decompressFrame already has */
            dst = (BYTE*)dst + res;
            dstCapacity -= res;
        }
    }  /* while (srcSize >= ZSTD1_frameHeaderSize_prefix) */
...
static int ZSTD1_isSkipFrame(ZSTD1_DCtx* dctx) { return dctx->stage == ZSTDds_skipFrame; }
...
{
    DEBUGLOG(5, "ZSTD1_decompressContinue (srcSize:%u)", (U32)srcSize);
    /* Sanity check */
    ...
    case ZSTDds_getFrameHeaderSize :
        assert(src != NULL);
        if (dctx->format == ZSTD1_f_zstd1) {  /* allows header */
            assert(srcSize >= ZSTD1_frameIdSize);  /* to read skippable magic number */
            if ((MEM_readLE32(src) & 0xFFFFFFF0U) == ZSTD1_MAGIC_SKIPPABLE_START) {        /* skippable frame */
                memcpy(dctx->headerBuffer, src, srcSize);
                dctx->expected = ZSTD1_skippableHeaderSize - srcSize;  /* remaining to load to get full skippable frame header */
                dctx->stage = ZSTDds_decodeSkippableHeader;
                return 0;
        }   }
        dctx->headerSize = ZSTD1_frameHeaderSize_internal(src, srcSize, dctx->format);
    ...
    case ZSTDds_decompressLastBlock:
    case ZSTDds_decompressBlock:
        DEBUGLOG(5, "ZSTD1_decompressContinue: case ZSTDds_decompressBlock");
        {   size_t rSize;
            switch(dctx->bType)
            {
            case bt_compressed:
                DEBUGLOG(5, "ZSTD1_decompressContinue: case bt_compressed");
                rSize = ZSTD1_decompressBlock_internal(dctx, dst, dstCapacity, src, srcSize, /* frame */ 1);
                break;
            case bt_raw :
            ...
            DEBUGLOG(5, "ZSTD1_decompressContinue: decoded size from block : %u", (U32)rSize);
            dctx->decodedSize += rSize;
            if (dctx->fParams.checksumFlag) XXH64_update(&dctx->xxhState, dst, rSize);

            if (dctx->stage == ZSTDds_decompressLastBlock) {   /* end of frame */
                DEBUGLOG(4, "ZSTD1_decompressContinue: decoded size from frame : %u", (U32)dctx->decodedSize);
                if (dctx->fParams.frameContentSize != ZSTD1_CONTENTSIZE_UNKNOWN) {
    ...
    case ZSTDds_checkChecksum:
        ...
        {   U32 const h32 = (U32)XXH64_digest(&dctx->xxhState);
            ...
            DEBUGLOG(4, "ZSTD1_decompressContinue: checksum : calculated %08X :: %08X read", h32, check32);
    ...
    case ZSTDds_decodeSkippableHeader:
        assert(src != NULL);
        assert(srcSize <= ZSTD1_skippableHeaderSize);
        memcpy(dctx->headerBuffer + (ZSTD1_skippableHeaderSize - srcSize), src, srcSize);   /* complete skippable header */
        dctx->expected = MEM_readLE32(dctx->headerBuffer + ZSTD1_frameIdSize);   /* note : dctx->expected can grow seriously large, beyond local buffer size */
...
static size_t ZSTD1_loadEntropy(ZSTD1_entropyDTables_t* entropy, const void* const dict, size_t const dictSize)
{
    ...
    {   short offcodeNCount[MaxOff+1];
        ...
        ZSTD1_buildFSETable(entropy->OFTable,
    ...
    {   short matchlengthNCount[MaxML+1];
        ...
        ZSTD1_buildFSETable(entropy->MLTable,
    ...
    {   short litlengthNCount[MaxLL+1];
        ...
        ZSTD1_buildFSETable(entropy->LLTable,
...
static size_t ZSTD1_decompress_insertDictionary(ZSTD1_DCtx* dctx, const void* dict, size_t dictSize)
{
    ...
    dctx->dictID = MEM_readLE32((const char*)dict + ZSTD1_frameIdSize);
...
{
    assert(dctx != NULL);
    dctx->expected = ZSTD1_startingInputLength(dctx->format);  /* dctx->format must be properly set */
    dctx->stage = ZSTDds_getFrameHeaderSize;
    dctx->decodedSize = 0;
    ...
    dctx->dictID = 0;
    ZSTD1_STATIC_ASSERT(sizeof(dctx->entropy.rep) == sizeof(repStartValue));
...
size_t ZSTD1_decompressBegin_usingDict(ZSTD1_DCtx* dctx, const void* dict, size_t dictSize)
{
    ...
    if (dict && dictSize)
        CHECK_E(ZSTD1_decompress_insertDictionary(dctx, dict, dictSize), dictionary_corrupted);
    return 0;
}
...
static size_t ZSTD1_loadEntropy_inDDict(ZSTD1_DDict* ddict, ZSTD1_dictContentType_e dictContentType)
{
    ddict->dictID = 0;
    ddict->entropyPresent = 0;
    if (dictContentType == ZSTD1_dct_rawContent) return 0;
    ...
    ddict->dictID = MEM_readLE32((const char*)ddict->dictContent + ZSTD1_frameIdSize);
...
static size_t ZSTD1_initDDict_internal(ZSTD1_DDict* ddict,
                                      const void* dict, size_t dictSize,
                                      ZSTD1_dictLoadMethod_e dictLoadMethod,
                                      ZSTD1_dictContentType_e dictContentType)
{
    if ((dictLoadMethod == ZSTD1_dlm_byRef) || (!dict) || (!dictSize)) {
    ...
    CHECK_F( ZSTD1_loadEntropy_inDDict(ddict, dictContentType) );

    return 0;
}

ZSTD1_DDict* ZSTD1_createDDict_advanced(const void* dict, size_t dictSize,
                                      ZSTD1_dictLoadMethod_e dictLoadMethod,
                                      ZSTD1_dictContentType_e dictContentType,
                                      ZSTD1_customMem customMem)
{
    ...
    {   ZSTD1_DDict* const ddict = (ZSTD1_DDict*) ZSTD1_malloc(sizeof(ZSTD1_DDict), customMem);
        ...
        if (ZSTD1_isError( ZSTD1_initDDict_internal(ddict, dict, dictSize, dictLoadMethod, dictContentType) )) {
...
ZSTD1_DDict* ZSTD1_createDDict(const void* dict, size_t dictSize)
{
    ...
    return ZSTD1_createDDict_advanced(dict, dictSize, ZSTD1_dlm_byCopy, ZSTD1_dct_auto, allocator);
}
...
{
    ZSTD1_customMem const allocator = { NULL, NULL, NULL };
    return ZSTD1_createDDict_advanced(dictBuffer, dictSize, ZSTD1_dlm_byRef, ZSTD1_dct_auto, allocator);
}


const ZSTD1_DDict* ZSTD1_initStaticDDict(
                                ...
                                ZSTD1_dictContentType_e dictContentType)
{
    size_t const neededSpace =
            sizeof(ZSTD1_DDict) + (dictLoadMethod == ZSTD1_dlm_byRef ? 0 : dictSize);
    ...
    if (ZSTD1_isError( ZSTD1_initDDict_internal(ddict, dict, dictSize, ZSTD1_dlm_byRef, dictContentType) ))
        return NULL;
    return ddict;
}
 ...
 *  Note : dictionary created by reference using ZSTD1_dlm_byRef are smaller */
size_t ZSTD1_estimateDDictSize(size_t dictSize, ZSTD1_dictLoadMethod_e dictLoadMethod)
{
    return sizeof(ZSTD1_DDict) + (dictLoadMethod == ZSTD1_dlm_byRef ? 0 : dictSize);
}
...
unsigned ZSTD1_getDictID_fromDict(const void* dict, size_t dictSize)
{
    ...
    return MEM_readLE32((const char*)dict + ZSTD1_frameIdSize);
}
...
{
    ZSTD1_frameHeader zfp = { 0, 0, 0, ZSTD1_frame, 0, 0, 0 };
...
{
    DEBUGLOG(3, "ZSTD1_createDStream");
...
size_t ZSTD1_DCtx_loadDictionary_advanced(ZSTD1_DCtx* dctx, const void* dict, size_t dictSize, ZSTD1_dictLoadMethod_e dictLoadMethod, ZSTD1_dictContentType_e dictContentType)
{
    if (dctx->streamStage != zdss_init) return ERROR(stage_wrong);
    ZSTD1_freeDDict(dctx->ddictLocal);
    if (dict && dictSize >= 8) {
        dctx->ddictLocal = ZSTD1_createDDict_advanced(dict, dictSize, dictLoadMethod, dictContentType, dctx->customMem);
...
size_t ZSTD1_DCtx_loadDictionary_byReference(ZSTD1_DCtx* dctx, const void* dict, size_t dictSize)
{
    return ZSTD1_DCtx_loadDictionary_advanced(dctx, dict, dictSize, ZSTD1_dlm_byRef, ZSTD1_dct_auto);
}

size_t ZSTD1_DCtx_loadDictionary(ZSTD1_DCtx* dctx, const void* dict, size_t dictSize)
{
    return ZSTD1_DCtx_loadDictionary_advanced(dctx, dict, dictSize, ZSTD1_dlm_byCopy, ZSTD1_dct_auto);
}

size_t ZSTD1_DCtx_refPrefix_advanced(ZSTD1_DCtx* dctx, const void* prefix, size_t prefixSize, ZSTD1_dictContentType_e dictContentType)
{
    return ZSTD1_DCtx_loadDictionary_advanced(dctx, prefix, prefixSize, ZSTD1_dlm_byRef, dictContentType);
}

size_t ZSTD1_DCtx_refPrefix(ZSTD1_DCtx* dctx, const void* prefix, size_t prefixSize)
{
    return ZSTD1_DCtx_refPrefix_advanced(dctx, prefix, prefixSize, ZSTD1_dct_rawContent);
}


/* ZSTD1_initDStream_usingDict() :
 * return : expected size, aka ZSTD1_frameHeaderSize_prefix.
 * this function cannot fail */
size_t ZSTD1_initDStream_usingDict(ZSTD1_DStream* zds, const void* dict, size_t dictSize)
{
    DEBUGLOG(4, "ZSTD1_initDStream_usingDict");
    zds->streamStage = zdss_init;
    CHECK_F( ZSTD1_DCtx_loadDictionary(zds, dict, dictSize) );
...
{
    DEBUGLOG(4, "ZSTD1_initDStream");
    return ZSTD1_initDStream_usingDict(zds, NULL, 0);
}

size_t ZSTD1_DCtx_refDDict(ZSTD1_DCtx* dctx, const ZSTD1_DDict* ddict)
{
    if (dctx->streamStage != zdss_init) return ERROR(stage_wrong);
    dctx->ddict = ddict;
    return 0;
 ...
 * this function cannot fail */
size_t ZSTD1_initDStream_usingDDict(ZSTD1_DStream* dctx, const ZSTD1_DDict* ddict)
{
    size_t const initResult = ZSTD1_initDStream(dctx);
    dctx->ddict = ddict;
    return initResult;
}
 ...
 * return : expected size, aka ZSTD1_frameHeaderSize_prefix.
 * this function cannot fail */
size_t ZSTD1_resetDStream(ZSTD1_DStream* dctx)
{
    DEBUGLOG(4, "ZSTD1_resetDStream");
...
size_t ZSTD1_setDStreamParameter(ZSTD1_DStream* dctx,
...
size_t ZSTD1_DCtx_setMaxWindowSize(ZSTD1_DCtx* dctx, size_t maxWindowSize)
{
    if (dctx->streamStage != zdss_init) return ERROR(stage_wrong);
...
size_t ZSTD1_DCtx_setFormat(ZSTD1_DCtx* dctx, ZSTD1_format_e format)
{
    DEBUGLOG(4, "ZSTD1_DCtx_setFormat : %u", (unsigned)format);
    if (dctx->streamStage != zdss_init) return ERROR(stage_wrong);
    dctx->format = format;
    return 0;
}
...
size_t ZSTD1_sizeof_DStream(const ZSTD1_DStream* dctx)
{
    return ZSTD1_sizeof_DCtx(dctx);
}

size_t ZSTD1_decodingBufferSize_min(unsigned long long windowSize, unsigned long long frameContentSize)
{
    size_t const blockSize = (size_t) MIN(windowSize, ZSTD1_BLOCKSIZE_MAX);
...
size_t ZSTD1_estimateDStreamSize(size_t windowSize)
{
    ...
    size_t const outBuffSize = ZSTD1_decodingBufferSize_min(windowSize, ZSTD1_CONTENTSIZE_UNKNOWN);
    return ZSTD1_estimateDCtxSize() + inBuffSize + outBuffSize;
}

size_t ZSTD1_estimateDStreamSize_fromFrame(const void* src, size_t srcSize)
{
    U32 const windowSizeMax = 1U << ZSTD1_WINDOWLOG_MAX;   /* note : should be user-selectable */
    ZSTD1_frameHeader zfh;
    size_t const err = ZSTD1_getFrameHeader(&zfh, src, srcSize);
    if (ZSTD1_isError(err)) return err;
    if (err>0) return ERROR(srcSize_wrong);
    if (zfh.windowSize > windowSizeMax)
        return ERROR(frameParameter_windowTooLarge);
    return ZSTD1_estimateDStreamSize((size_t)zfh.windowSize);
}
...
size_t ZSTD1_decompressStream(ZSTD1_DStream* zds, ZSTD1_outBuffer* output, ZSTD1_inBuffer* input)
{
    ...
    while (someMoreWork) {
        ...
        case zdss_loadHeader :
            ...
            if (zds->legacyVersion) {
                ...
                {   size_t const hint = ZSTD1_decompressLegacyStream(zds->legacyContext, zds->legacyVersion, output, input);
                    if (hint==0) zds->streamStage = zdss_init;
                    return hint;
            }   }
#endif
            {   size_t const hSize = ZSTD1_getFrameHeader_internal(&zds->fParams, zds->headerBuffer, zds->lhSize, zds->format);
                ...
                if (ZSTD1_isError(hSize)) {
                    ...
                    if (legacyVersion) {
                        ...
                        DEBUGLOG(5, "ZSTD1_decompressStream: detected legacy version v0.%u", legacyVersion);
                        /* legacy support is incompatible with static dctx */
                        if (zds->staticSize) return ERROR(memory_allocation);
                        CHECK_F(ZSTD1_initLegacyStream(&zds->legacyContext,
                                    zds->previousLegacyVersion, legacyVersion,
                                    dict, dictSize));
                        zds->legacyVersion = zds->previousLegacyVersion = legacyVersion;
                        {   size_t const hint = ZSTD1_decompressLegacyStream(zds->legacyContext, legacyVersion, output, input);
            ...
            if (zds->fParams.frameContentSize && zds->fParams.windowSize /* skippable frame if == 0 */
                ...
                if (cSize <= (size_t)(iend-istart)) {
                    ...
                    DEBUGLOG(4, "shortcut to single-pass ZSTD1_decompress_usingDDict()")
                    ip = istart + cSize;
                    op += decompressedSize;
                    zds->expected = 0;
                    zds->streamStage = zdss_init;
            ...
            if ((MEM_readLE32(zds->headerBuffer) & 0xFFFFFFF0U) == ZSTD1_MAGIC_SKIPPABLE_START) {  /* skippable frame */
                zds->expected = MEM_readLE32(zds->headerBuffer + ZSTD1_frameIdSize);
            ...
            {   size_t const neededInBuffSize = MAX(zds->fParams.blockSizeMax, 4 /* frame checksum */);
                size_t const neededOutBuffSize = ZSTD1_decodingBufferSize_min(zds->fParams.windowSize, zds->fParams.frameContentSize);
    ...
    {   size_t nextSrcSizeHint = ZSTD1_nextSrcSizeToDecompress(zds);
        ...
        nextSrcSizeHint += ZSTD1_blockHeaderSize * (ZSTD1_nextInputType(zds) == ZSTDnit_block);   /* preload header of next block */
...
size_t ZSTD1_decompress_generic(ZSTD1_DCtx* dctx, ZSTD1_outBuffer* output, ZSTD1_inBuffer* input)
{
    return ZSTD1_decompressStream(dctx, output, input);
}

size_t ZSTD1_decompress_generic_simpleArgs (
                            ZSTD1_DCtx* dctx,
                      ...
                      const void* src, size_t srcSize, size_t* srcPos)
{
    ...
    size_t const cErr = ZSTD1_decompress_generic(dctx, &output, &input);
    *dstPos = output.pos;
    *srcPos = input.pos;
    return cErr;
}

void ZSTD1_DCtx_reset(ZSTD1_DCtx* dctx)
{
    (void)ZSTD1_initDStream(dctx);
    dctx->format = ZSTD1_f_zstd1;
    dctx->maxWindowSize = ZSTD1_MAXWINDOWSIZE_DEFAULT;
}
